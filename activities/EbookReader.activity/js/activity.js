...
var app = new Vue({
              ...
              .loadAsText(function (error, metadata, data) {
                if (error == null && data != null) {
                  ...
                  if (parsed.current !== undefined) {
                    ...
                    vm.currentEpub = ePub(
                      vm.currentLibrary.information.fileprefix +
                        vm.currentBook.file
                    );
  ...
  methods: {
    ...
    fullscreen: function () {
      ...
      document.getElementById("unfullscreen-button").style.visibility =
        "visible";
      if (this.currentView === EbookReader) {
        ...
        content:
          `
					<div id='popup-toolbar' class='toolbar' style='padding: 0'>
						<button class='toolbutton pull-right' id='popup-ok-button' title='` +
          titleOk +
          `' style='outline:none;background-image: url(lib/sugar-web/graphics/icons/actions/dialog-ok.svg)'></button>
						<button class='toolbutton pull-right' id='popup-cancel-button' title='` +
          titleCancel +
          `' style='outline:none;background-image: url(lib/sugar-web/graphics/icons/actions/dialog-cancel.svg)'></button>
    ...
    settingsShown: function () {
      ...
      document.getElementById("popup-container").style.height =
        document.getElementById("popup-toolbar").parentNode.offsetHeight -
        55 * 2 +
        "px";
      document
        .getElementById("popup-ok-button")
        .addEventListener("click", function () {
    ...
    onHelp: function () {
      ...
      if (
        this.currentView === LibraryViewer &&
        this.$refs.view.$refs.item0 &&
        this.$refs.view.$refs.item0[0]
      ) {
    ...
    saveContextToJournal: function () {
      ...
      requirejs(["sugar-web/activity/activity"], function (activity) {
        ...
        var context = {
          library: {
            information: vm.currentLibrary.information,
            database: vm.currentLibrary.database,
          },
        };
        if (vm.currentView === EbookReader) {
